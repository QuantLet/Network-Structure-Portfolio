...
# 1.modify the boxplot of different weights
#
...
setwd("~/同步空间/Network_structure_based_portfolio/Dantzig-selector estimation covariance/Daily_rebalance_20230425")
...
date_daily<-as.Date.factor(date[(0:1317)*1+1], format="%Y-%m-%d")
...
##### Load portfolios #####
load("Portfolios_20230425.RData")
...
negative_weights<-zoo(data.frame(nega.weights.plugin),
                      order.by=as.Date(as.character(date_daily), format="%Y-%m-%d"))
